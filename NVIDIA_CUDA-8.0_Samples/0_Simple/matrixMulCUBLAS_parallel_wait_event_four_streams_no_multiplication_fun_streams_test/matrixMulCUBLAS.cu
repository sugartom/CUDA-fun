...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size)
{
    ...
    // to measure enqueue overhead
    clock_t *enq_start = (clock_t *) malloc(nIter * sizeof(clock_t));
    clock_t *enq_end = (clock_t *) malloc(nIter * sizeof(clock_t));
    double *elapse_secs = (double *) malloc(nIter * sizeof(double));
    double *time_between = (double *) malloc((nIter - 1) * sizeof(double));
    ...
    float useless_time = 1000;
    float event_time = 0.01;
    float manual_time = 1;
    float kernel_time = 0.01; // time the kernel should run in ms // ********************
    clock_t useless_clocks = (clock_t)(useless_time * deviceProp.clockRate);
    clock_t event_clocks = (clock_t)(event_time * deviceProp.clockRate);
    clock_t manual_clocks = (clock_t)(manual_time * deviceProp.clockRate);
    ...
    int useless_nbytes = nIter * sizeof(clock_t);
    clock_t *useless_da1 = 0;
    checkCudaErrors(cudaMalloc((void **)&useless_da1, useless_nbytes));
    clock_t *useless_da2 = 0;
    checkCudaErrors(cudaMalloc((void **)&useless_da2, useless_nbytes));
    clock_t *useless_da3 = 0;
    checkCudaErrors(cudaMalloc((void **)&useless_da3, useless_nbytes));
    clock_t *useless_da4 = 0;
    checkCudaErrors(cudaMalloc((void **)&useless_da4, useless_nbytes));
    clock_t *useless_da5 = 0;
    checkCudaErrors(cudaMalloc((void **)&useless_da5, useless_nbytes));
    clock_t *useless_da6 = 0;
    checkCudaErrors(cudaMalloc((void **)&useless_da6, useless_nbytes));
    ...
    // CUBLAS version 2.0
    {
		...
		// tom
        clock_block<<<1,1,0,stream2>>>(&d_a[0], useless_clocks);
        ...
        for (int j = 0; j < nIter; j++)
        {
			...
			clock_block<<<1,1,0,stream3>>>(&useless_da3[j], time_clocks);
            enq_end[j] = clock();
            ...
            clock_block<<<1,1,0,stream4>>>(&useless_da4[j], time_clocks);
        }

        checkCudaErrors(cudaStreamWaitEvent(stream5, tomEvent[0],0));
        clock_block<<<1,1,0,stream5>>>(&useless_da5[0], manual_clocks);
        checkCudaErrors(cudaEventRecord(tomEvent[3], stream5));
        checkCudaErrors(cudaStreamWaitEvent(stream3, tomEvent[3],0));

        // clock_block<<<1,1,0,stream5>>>(&useless_da5[0], time_clocks);
        checkCudaErrors(cudaStreamWaitEvent(stream3, tomEvent[2],0));
        ...
        for (int j = 0; j < nIter; j++)
        {
            ...
            // checkCudaErrors(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, matrix_size.uiWB, matrix_size.uiHA, matrix_size.uiWA, &alpha, d_B, matrix_size.uiWB, d_A, matrix_size.uiWA, &beta, d_C, matrix_size.uiWB));
            clock_block<<<1,1,0,stream1>>>(&useless_da1[j], time_clocks);
            ...
            clock_block<<<1,1,0,stream2>>>(&useless_da2[j], time_clocks);
        }

        clock_block<<<1,1,0,stream6>>>(&useless_da6[0], event_clocks);
        checkCudaErrors(cudaEventRecord(tomEvent[2], stream6));
    ...
    for (int i = 0; i < nIter; i++) {
        elapse_secs[i] = double(enq_end[i] - enq_start[i]) / CLOCKS_PER_SEC;
        printf("elapse_secs[%d] = %fs\n", i, elapse_secs[i]);
    }

    for (int i = 0; i < (nIter - 1); i++) {
        time_between[i] = double(enq_start[i + 1] - enq_start[i]) / CLOCKS_PER_SEC;
        printf("time_between[%d] = %fs\n", i, time_between[i]);
    }
