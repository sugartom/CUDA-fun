...
for i in range(len(input_list)):
	...
	GFLOPS_wo_streams.append(float(f[29].rstrip().split()[-1].split("=")[-1]))
	# print GFLOPS_wo_streams
	GFLOPS_w_streams.append(float(f[42].rstrip().split()[-1].split("=")[-1]))
	GFLOPS_batched.append(float(f[55].rstrip().split()[-1].split("=")[-1]))
...
plt.xscale('log')
plt.yscale('log')
plt.savefig('fig/plot_batchCUBLAS_d.png')
plt.show()