...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size)
{
    ...
    int eventNum = 2;
    int waitNum = 2;
    cudaEvent_t *tomEvent = (cudaEvent_t *) malloc(eventNum * sizeof(cudaEvent_t));
    for (int i = 0; i < eventNum; i++) {
        checkCudaErrors(cudaEventCreate(&tomEvent[i]));
    }
    float kernel_time = 1000; // time the kernel should run in ms
    clock_t time_clocks = (clock_t)(kernel_time * deviceProp.clockRate);
    int nbytes = waitNum * sizeof(clock_t);   // number of data bytes
    ...
    // CUBLAS version 2.0
    {
        ...
        clock_block<<<1,1,0,stream1>>>(&d_a[0], 0.000001 * time_clocks);
        ...
        clock_block<<<1,1,0,stream1>>>(&d_a[1], 0.000001 * time_clocks);
        // checkCudaErrors(cudaEventRecord(tomEvent[1], stream1));
        // tom
