...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size)
{
    ...
    // CUBLAS version 2.0
    {
        ...
        clock_block<<<1,1,0,stream3>>>(&d_a[3], 2 * time_clocks);
        checkCudaErrors(cudaEventRecord(tomEvent[2],stream3));

        clock_block<<<1,1,0,stream2>>>(&d_a[2], 0.5*time_clocks);
        checkCudaErrors(cudaStreamWaitEvent(stream2, tomEvent[2],0));
        checkCudaErrors(cudaEventRecord(tomEvent[1],stream2));
        checkCudaErrors(cudaStreamWaitEvent(stream1, tomEvent[1],0));
        clock_block<<<1,1,0,stream1>>>(&d_a[1], time_clocks);
